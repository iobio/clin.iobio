...
export default class TreeBuilder {
  ...
  create() {
    ...
    // Compute the layout.
    this.tree = d3.tree().nodeSize([nodeSize[0] * 2, nodeSize[1] * 3.5]);
  ...
  _update(source) {
    ...
    const linkEls = this.svg.selectAll(".link").data(links);
    ...
    const sibEls = this.svg.selectAll(".sibling").data(this.siblings);
    ...
    const scaledMinSize = d =>
      (1 / Math.sqrt(2)) * d3.min([d.cWidth, d.cHeight]);
  ...
  _siblingLine(d, i) {
    let ny = d.target.y + (d.source.y - d.target.y) * 0.5;
