export default class MosaicSession {
  ...
  promiseInit(sampleId, source, isPedigree, projectId, clientAppId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self.promiseGetSampleInfo(projectId, sampleId, isPedigree).then(data => {
        ...
        self
          .promiseGetFileMapForSample(projectId, probandSample, "proband")
          .then(data => {
          ...
          .then(() => {
            for (var rel in pedigree) {
              if (rel != "unparsed") {
                ...
                samples.forEach(s => {
                  let p = self
                    .promiseGetFileMapForSample(projectId, s, rel)
                    .then(data => {
                      ...
                      let coverageHisto = {
                        id: sampleId,
                        coverage:
                          theSample.distributions.coverage_hist_no_outliers
                      };
                      let varCounts = {
                        id: sampleId,
                        counts: {
                          SNP: theSample.metrics.var_snp_count,
                          indel: theSample.metrics.var_indel_count,
                          other: theSample.metrics.var_other_count
                        }
                      };
                      ...
                      if (!bypass) {
                        var modelInfo = {
                          relationship:
                            data.relationship == "siblings"
                              ? "sibling"
                              : data.relationship,
                          affectedStatus: isPedigree
                            ? theSample.pedigree.affection_status == 2
                              ? "affected"
                              : "unaffected"
                            : "affected",
                          sex: isPedigree
                            ? theSample.pedigree.sex == 1
                              ? "male"
                              : theSample.pedigree.sex == 2
                              ? "female"
                              : "unknown"
                            : "unknown",
                          name: theSample.name,
                          sample: theSample.files.vcf
                            ? theSample.vcf_sample_name
                            : theSample.name,
                          vcf: theSample.files.vcf,
                          tbi:
                            theSample.files.tbi == null ||
                            theSample.files.tbi.indexOf(theSample.files.vcf) ==
                              0
                              ? null
                              : theSample.files.tbi,
                ...
                modelInfos.forEach(function(modelInfo) {
                  if (modelInfo.sample == null || modelInfo.sample == "") {
                    buf +=
                      ...
                      ")   has an empty vcf_sample_name. Unable to properly filter variants for this sample.<br><br>";
                  }
                });
                if (buf.length > 0) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (proband && proband.length > 0) {
        ...
        fileInfos.forEach(function(fileInfo) {
          let p = self
            .promiseParseVariantSetFile(fileInfo, proband[0])
            .then(function(data) {
              if (data) {
              ...
              if (fields.length >= self.variantSetTxtCols.length - 1) {
                ...
                if (fields.length == self.variantSetTxtCols.length) {
                  if (
                    variant.sampleId &&
                    modelInfo.sample &&
                    variant.sampleId != modelInfo.sample
                  ) {
                    keep = false;
                  }
                }
                if (keep) {
                  if (
                    variant.gene == "" ||
                    variant.gene == null ||
                    variant.gene.trim().length == 0
                  ) {
                    console.log(
                      "promiseParseVariantSets: missing gene field.  bypassing record " +
                        record
                    );
                  } else {
                  ...
                  console.log(
                    "bypassing variant rec for sample " + variant.sampleId
                  );
                }
              } else {
                console.log(
                  "promiseParseVariantSets: insufficient record fields.  bypassinging record " +
                    record
                );
              }
            });
  ...
  promiseGetPedigreeForSample(project_id, sample_id) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      self
        ...
        .fail(error => {
          reject(
            "Error getting pedigree for sample " + sample_id + ": " + error
          );
        });
    });
  }
  ...
  parsePedigree(raw_pedigree, sample_id) {
    ...
    let probandIndex = raw_pedigree.findIndex(
      d => d.id == sample_id && d.pedigree.maternal_id && d.pedigree.paternal_id
    );
    ...
    if (probandIndex == -1) {
      probandIndex = raw_pedigree.findIndex(
        d =>
          d.pedigree.affection_status == 2 &&
          d.pedigree.maternal_id &&
          d.pedigree.paternal_id
      );
    }
    // If the sample selected doesn't have a mother and father (isn't a proband), find
    ...
    if (probandIndex != -1) {
      ...
      // Get mother
      const motherIndex = raw_pedigree.findIndex(
        d => d.id == proband.pedigree.maternal_id
      );
      if (motherIndex != -1) {
      ...
      // Get mother
      const fatherIndex = raw_pedigree.findIndex(
        d => d.id == proband.pedigree.paternal_id
      );
      if (fatherIndex != -1) {
      ...
      alertify.alert(
        "Error",
        "Could not load the trio.  Unable to identify a proband (offspring) from this pedigree."
      );
      return null;
    }
    ...
    raw_pedigree.forEach(sample => {
      if (
        sample.pedigree.maternal_id != null ||
        (sample.pedigree.paternal_id != null &&
  ...
  promiseGetFileMapForSample(project_id, sample, relationship) {
    ...
    return new Promise((resolve, reject) => {
      ...
      self
        .promiseGetFilesForSample(project_id, currentSample.id)
        .then(files => {
          files
            .filter(file => {
              return file.type;
            ...
            .forEach(file => {
              var p = self
                .promiseGetSignedUrlForFile(project_id, currentSample.id, file)
                .then(signed => {
                  if (file.type == "txt" || file.type == "tsv") {
                    ...
                    if (file.type == "vcf") {
                      if (
                        file.vcf_sample_name == null ||
                        file.vcf_sample_name == ""
                      ) {
                        alertify.error(
                          "Missing vcf_sample_name for file " + file.name,
                          20
                        );
                      } else {
  ...
  promiseAddAnalysis(projectId, analysis) {
    ...
    return new Promise(function(resolve, reject) {
      self
        ...
        .fail(error => {
          reject(
            "Error adding analysis for project " + projectId + ": " + error
          );
        });
    });
  }
  ...
  promiseUpdateAnalysis(analysis) {
    ...
    return new Promise(function(resolve, reject) {
      self
        .updateAnalysis(analysis.project_id, analysis.id, analysis)
        .done(response => {
          resolve(response);
        })
        .fail(error => {
  ...
  promiseUpdateAnalysisTitle(analysis) {
    ...
    return new Promise(function(resolve, reject) {
      self
        .updateAnalysisTitle(analysis.project_id, analysis.id, analysis)
        .done(response => {
          resolve(response);
        })
        .fail(error => {
  ...
  addAnalysis(projectId, newAnalysisData) {
    ...
    return $.ajax({
      url:
        ...
        "/projects/" +
        projectId +
        "/analyses/?client_application_id=" +
        this.client_application_id,
      type: "POST",
