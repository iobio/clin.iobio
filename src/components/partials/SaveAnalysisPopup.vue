...
<template>
  <v-dialog
    content-class="save-analysis-dialog"
    width="400"
    persistent
    v-model="showPopup"
  >
    <v-card v-if="analysis" class="save-analysis-content full-width">
      <v-card-title style="justify-content: space-between;padding: 0px;">
        <span class="info-title">
          {{
            analysis.id
              ? "Name and description of this analysis"
              : "Add this analysis to Mosaic"
          }}</span
        >
      ...
      <v-layout row wrap>
        ...
        <v-flex class="mt-4" style="display:flex;justify-content:flex-end" xs12>
          <v-btn class="primary mr-2" @click="onSave">{{
            analysis.id ? "Apply" : "Save"
          }}</v-btn>
