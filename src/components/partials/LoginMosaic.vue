      ...
      <v-card light>
        <v-layout row wrap>
          <v-flex xs12>
            <v-text-field
              v-model="researcher"
              label="Enter clin.iobio user name"
              type="text"
            >
            </v-text-field>
          </v-flex>
          ...
          <v-flex v-if="!isAuthenticated" xs12>
            <v-text-field
              v-model="password"
              label="Enter clin.iobio password"
              type="password"
            >
            </v-text-field>
          </v-flex>
          ...
          <v-flex xs12 v-if="!isAuthenticated">
            <v-btn
              :disabled="researcher == null || password == null ? true : false"
              @click="authenticate"
              >Launch clin.iobio</v-btn
            >
          </v-flex>
        </v-layout>
      </v-card>
...
export default {
  ...
  methods: {
    authenticate: function() {
      let self = this;
      self.awsSession.authenticateMosaic(
        self.researcher,
        self.password,
        function(success) {
          if (success) {
