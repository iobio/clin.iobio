  ...
  <div :style="`height: ${height}px`">
    <svg v-if="showChart" class="coverage-bar-chart">
      <g class="g-main">
        ...
        <g class="scores" />
        <g class="brush">
          <text class="extent extent--e" text-anchor="start" />
          <text class="extent extent--w" text-anchor="end" />
        </g>
      </g>
...
export default {
  ...
  computed: {
    ...
    xDomainLocal() {
      ...
      return [
        this.typedData[0][0],
        this.typedData[this.typedData.length - 1][0] + extend
      ];
    },
    yDomainLocal() {
    ...
    xRange() {
      return [
        this.margin.left + this.labelHeight,
    ...
    xRangeNorm() {
      return [
        this.margin.left + this.labelHeight,
        this.widthNorm - this.margin.right
      ];
    },

    yRange() {
      return [
        this.height - this.margin.bottom - this.labelHeight,
        this.margin.top
      ];
    }
  },
  ...
  methods: {
    ...
    createAxis() {
      ...
      this.yScale = d3
        ...
        .domain(this.yDomain ? this.yDomain : this.yDomainLocal)
        .nice()
        .range(this.yRange);
      const xAxis = d3
        .axisBottom()
      ...
      this.gMain
        ...
        .attr(
          "transform",
          `translate(${this.margin.left + this.labelHeight} , 0)`
        )
        .call(yAxis);
    },
    addLabels() {
    ...
    drawBars() {
      const dataJoin = this.gMain
        .select(".scores")
        .selectAll("rect")
        .data(this.typedData, d => d);
      dataJoin
        .enter()
        .append("rect")
        .merge(dataJoin)
          ...
          () =>
            Math.abs(
              (this.xRange[1] - this.xRange[0]) /
                (this.typedData.length + this.xDiff)
            ) - this.barPadding
        )
        .attr(
          "height",
          d => this.yScale(this.yScale.domain()[0]) - this.yScale(d[1])
        );
    ...
    addBrush() {
      if (this.canFilter) {
        this.brush = d3
          .brushX()
          .extent([
            [this.xRange[0], this.yRange[1]],
            [this.xRange[1], this.yRange[0]]
          ])
          .on("brush", this.updateBrush)
          .on("end", this.localBrushEnd);
        this.gMain.select(".brush").call(this.brush);
      }
    },
    updateBrushExtent() {
      if (this.canFilter) {
    ...
    moveBrush() {
      ...
      if (extentRange) {
        ...
        const right =
          xVals[
            Math.min(d3.bisectLeft(xVals, extentDomain[1]), xVals.length - 1)
          ];
        ...
        const eastPos = [
          this.gMain.select(".handle--e").attr("x"),
          this.gMain.select(".handle--e").attr("y")
        ];
        const westPos = [
          this.gMain.select(".handle--w").attr("x"),
          this.gMain.select(".handle--w").attr("y")
        ];
