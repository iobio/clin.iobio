...
<template>
  <div id="review-case-panel">
    <div class="review-section">
      <div style="display:flex;flex-direction:row;justify-content:flex-start">
        ...
        <div v-if="false" class="subsection">
          ...
          <div
            v-for="(phenotype, index) in phenotypeList"
            class="phenotype-search-term"
          >
            {{ phenotype }}
          </div>
        </div>
    ...
    <div v-if="isSorted">
      <div
        style=" width: 100%; display: inline-flex; flex-direction: row; justify-content: space-around; padding-bottom: 10px"
      >
        <div class="heading" style="margin-right: 90px">Sample</div>
        <div
          class="heading"
          style="margin-right: 90px; display:flex;flex-direction:row;justify-content:space-between"
        >
      ...
      <div v-for="(d, i) in sampleIdsAndRelationships">
        <div
          style=" width: 100%; display: inline-flex; flex-direction: row; justify-content: space-around;"
        >
          <div style="text-align: center; width: 150px" class="capitalize">
            {{ sampleIdsAndRelationships[i] }}
            <PedigreeGraph
              :data="allPedigreeDataArrays[i]"
              :id="sampleUuids[i]"
              :width="100"
              :height="75"
              :pedigree="pedigree"
          ...
          <div style="display: inline-flex;">
            <BarChart
              :data="coverageDataArray[i]"
              :width="400"
              :height="150"
              :x-domain="xDomain"
              :y-domain="yDomain"
              :median-coverage="medianCoverages[i]"
              :minCutoff="minCutoff"
              :onHover="onHover"
            ></BarChart>
            ...
            <div style="padding-top: 20px" v-show="goodCoverage(i)">
              <v-tooltip top class="valign">
                ...
                <span
                  >Median coverage is above expected coverage threshold of
                  {{ minCutoff }}X</span
                >
              </v-tooltip>
              <div
                v-if="badCoverage"
                style=" display: inline-flex; width: 120px; line-height: 16px; font-size: 12px; padding-left: 5px;"
              ></div>
            </div>
            <div style="padding-top: 20px" v-show="!goodCoverage(i)">
              <v-icon
              ...
              <div
                v-if="badCoverage"
                style=" display: inline-flex; width: 120px; line-height: 14px; font-size: 13px; padding-left: 5px;"
              >
                Median coverage is below expected coverage threshold of
                {{ minCutoff }}X
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    populateReviewCaseBadges() {
      ...
      this.reviewCaseBadges = [
        { label: "samples (" + famName + ")", count: this.sampleIds.length }
      ];

      if (this.badCoverage) {
        this.reviewCaseBadges.push({
          label: "failed QC",
          count: this.badCoverageCount,
          class: "failed"
        });
      }
    ...
    calculateMedianCoverage(data) {
      ...
      let medianCoverage = this.findMedianFromCummulativeFrequencies(
        medianFreq,
        cumFreqs
      );
    ...
    overridePropsWithDemoData() {
      this.pedigreeData = {
        "0": {
          id: 3261,
          metrics: {
            second_mates: "16534422",
            test2_77850c8c: "15",
            proper_pairs: "99.1149",
            failed_qc: "0",
            strand_bias: "1.00551",
            ts_tv_ratio: "2.05896",
            variant_count: "88110",
            pedigree: "Pedigree",
            var_other_count: "4124",
            duplicate_rate: "0.00348412",
            affected_status: "Unaffected",
            file_location: "s3",
            both_mates_mapped: "32943377",
            var_snp_count: "77618",
            singletons: "55024",
            var_indel_count: "8672",
            first_mates: "16503521",
            total_reads: "33037943",
            mapped_reads: "32998401",
            read_coverage_iqr: "15",
            paired_end_reads: "33037943",
            median_read_coverage: "51"
          },
          distributions: {
            chrom_coverage: "null",
            mut_spec: {
            ...
            coverage_hist: { quartiles: [59, 51, 44], whiskers: [74, 29] },
            length_hist: { quartiles: [101, 101, 101], whiskers: [101, 101] },
            frag_hist: { quartiles: [363, 319, 277], whiskers: [449, 191] },
            mapq_hist: { quartiles: [60, 60, 60], whiskers: [60, 60] },
            coverage_hist_no_outliers: {
              "29": 0.004871,
              "30": 0.005242,
              "31": 0.005779,
              "32": 0.006489,
          ...
          ancestry: {
            background: 604,
            data: [-0.1026610208789221, -1.4242045534784185, "EUR"]
          },
          created_at: "2019-07-18T15:12:05.413Z",
          updated_at: "2019-07-18T15:12:05.413Z",
          kindred_id: 50,
          sample_id: 3261,
          paternal_id: 3263,
          maternal_id: 3264,
          ...
          client_name: "11002",
          pedigree: {
            kindred_id: "b537452d-9852-4605-8fd5-c90635d2824c",
            sample_id: 3261,
            paternal_id: 3263,
            maternal_id: 3264,
            sex: 1,
          ...
          vcf_sample_name: "NA12877",
          files: {
            bam: "https://s3.amazonaws.com/iobio/samples/bam/NA12877.exome.bam",
            tbi:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12877.vcf.gz.tbi",
            vcf:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12877.vcf.gz",
            bai:
              "https://s3.amazonaws.com/iobio/samples/bam/NA12877.exome.bam.bai"
          }
        },
        "1": {
          id: 3262,
          metrics: {
            duplicate_rate: "0.00414535",
            ts_tv_ratio: "2.14514",
            first_mates: "16282871",
            total_reads: "32568521",
            var_indel_count: "8816",
            var_other_count: "4006",
            singletons: "87747",
            file_location: "s3",
            strand_bias: "1.0064",
            mapped_reads: "32505279",
            second_mates: "16285650",
            pedigree: "Pedigree",
            median_read_coverage: "50",
            read_coverage_iqr: "13",
            variant_count: "86759",
            failed_qc: "0",
            proper_pairs: "98.8382",
            var_snp_count: "76210",
            paired_end_reads: "32568521",
          ...
          distributions: {
            ...
            coverage_hist: { quartiles: [57, 50, 44], whiskers: [70, 31] },
            length_hist: { quartiles: [101, 101, 101], whiskers: [101, 101] },
            frag_hist: { quartiles: [366, 321, 277], whiskers: [455, 188] },
            mapq_hist: { quartiles: [60, 60, 60], whiskers: [60, 60] },
            coverage_hist_no_outliers: {
              "31": 0.004288,
              "32": 0.005304,
          ...
          ancestry: {
            background: 604,
            data: [-0.31801863225785604, -1.4812542726924995, "EUR"]
          },
          created_at: "2019-07-18T15:12:05.413Z",
          updated_at: "2019-07-18T15:12:05.413Z",
          kindred_id: 50,
          ...
          vcf_sample_name: "NA12878",
          files: {
            bam: "https://s3.amazonaws.com/iobio/samples/bam/NA12878.exome.bam",
            bai:
              "https://s3.amazonaws.com/iobio/samples/bam/NA12878.exome.bam.bai",
            vcf:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12878.vcf.gz",
            tbi:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12878.vcf.gz.tbi"
          }
        },
        "2": {
          id: 3263,
          metrics: {
            duplicate_rate: "0.0093774",
            var_indel_count: "8622",
            total_reads: "32234312",
            var_snp_count: "76039",
            affected_status: "Unaffected",
            file_location: "s3",
            proper_pairs: "98.3934",
            second_mates: "16115247",
            median_read_coverage: "50",
            mapped_reads: "32190556",
            singletons: "61628",
            pedigree: "Pedigree",
            both_mates_mapped: "32128928",
            read_coverage_iqr: "13",
            ts_tv_ratio: "2.10934",
            var_other_count: "3940",
            strand_bias: "1.01811",
            failed_qc: "0",
            paired_end_reads: "32234312",
            variant_count: "86331",
            first_mates: "16119065"
          },
          distributions: {
            chrom_coverage: "null",
            mut_spec: {
            ...
            coverage_hist: { quartiles: [57, 50, 44], whiskers: [70, 31] },
            length_hist: { quartiles: [101, 101, 101], whiskers: [101, 101] },
            frag_hist: { quartiles: [342, 301, 263], whiskers: [421, 184] },
            mapq_hist: { quartiles: [60, 60, 60], whiskers: [60, 60] },
            coverage_hist_no_outliers: {
              "31": 0.005022,
              "32": 0.005582,
          ...
          ancestry: {
            background: 604,
            data: [-0.31890483438439093, -1.4071403255097141, "EUR"]
          },
          created_at: "2019-07-18T15:12:05.413Z",
          updated_at: "2019-07-18T15:12:05.413Z",
          kindred_id: 50,
          ...
          vcf_sample_name: "NA12891",
          files: {
            bam: "https://s3.amazonaws.com/iobio/samples/bam/NA12891.exome.bam",
            tbi:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12891.vcf.gz.tbi",
            vcf:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12891.vcf.gz",
            bai:
              "https://s3.amazonaws.com/iobio/samples/bam/NA12891.exome.bam.bai"
          }
        },
        "3": {
          id: 3264,
          metrics: {
            file_location: "s3",
            median_read_coverage: "54",
            affected_status: "Unaffected",
            var_snp_count: "76289",
            first_mates: "17447820",
            var_indel_count: "8644",
            read_coverage_iqr: "14",
            mapped_reads: "34860088",
            second_mates: "17456304",
            failed_qc: "0",
            total_reads: "34904124",
            ts_tv_ratio: "2.15153",
            proper_pairs: "98.8257",
            pedigree: "Pedigree",
            var_other_count: "3973",
            variant_count: "86702",
            singletons: "60944",
            strand_bias: "1.00962",
            duplicate_rate: "0.00513074",
            paired_end_reads: "34904124",
            both_mates_mapped: "34799144"
          },
          distributions: {
            chrom_coverage: "null",
            mut_spec: {
            ...
            coverage_hist: { quartiles: [61, 54, 47], whiskers: [75, 33] },
            length_hist: { quartiles: [101, 101, 101], whiskers: [101, 101] },
            frag_hist: { quartiles: [346, 304, 266], whiskers: [426, 186] },
            mapq_hist: { quartiles: [60, 60, 60], whiskers: [60, 60] },
            coverage_hist_no_outliers: {
              "33": 0.003492,
              "34": 0.004287,
          ...
          ancestry: {
            background: 604,
            data: [-0.2512083937845698, -1.4816045103941913, "EUR"]
          },
          created_at: "2019-07-18T15:12:05.413Z",
          updated_at: "2019-07-18T15:12:05.413Z",
          kindred_id: 50,
          ...
          vcf_sample_name: "NA12892",
          files: {
            bam: "https://s3.amazonaws.com/iobio/samples/bam/NA12892.exome.bam",
            bai:
              "https://s3.amazonaws.com/iobio/samples/bam/NA12892.exome.bam.bai",
            vcf:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12892.vcf.gz",
            tbi:
              "https://s3.amazonaws.com/iobio/samples/vcf/platinum-exome_NA12892.vcf.gz.tbi"
          }
        }
      };
      this.modelInfosData = [
      ...
      this.varCountsArray = [
        { id: "3261", counts: { SNP: 77618, indel: 8672, other: 4124 } },
        { id: "3261", counts: { SNP: 76289, indel: 8644, other: 3973 } },
        { id: "3261", counts: { SNP: 76039, indel: 8622, other: 3940 } },
        { id: "3261", counts: { SNP: 76210, indel: 8816, other: 4006 } }
      ];
    ...
    sortData() {
      ...
      this.pedigreeDataArray = tempPed.filter(function(el) {
        return el;
      });
      this.coverageDataArray = tempCoverage.filter(function(el) {
        return el;
      });
      this.varCountsArray = tempVarCounts.filter(function(el) {
        return el;
      });
      this.sampleIdsAndRelationships = tempSampleRelationship.filter(function(
        el
      ) {
        return el;
      });
      this.sampleIds = tempSamples.filter(function(el) {
        return el;
      });
      this.sampleUuids = tempUuids.filter(function(el) {
        return el;
      });
      this.medianCoverages = tempMedianCoverages.filter(function(el) {
        return el;
      });
    ...
    sortIndicesByRelationship() {
      ...
      this.sortedIndices = this.sortedIndices.filter(function(el) {
        return el !== null;
      });
    },
    ...
    populateRelationshipMap() {
      ...
      for (let i = 0; i < this.modelInfosData.length; i++) {
        this.sampleIdRelationshipMap[
          this.modelInfos[i].sample
        ] = this.modelInfos[i].relationship;
      }
    },
    ...
    formatPedDict(d) {
      let sampleIdRelationship =
        this.getRelationshipFromId(d.vcf_sample_name) +
        "\t" +
        d.vcf_sample_name;
