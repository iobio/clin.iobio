...
<template>
  <div id="findings-panel">
    ...
    <div
      v-if="clinicalNotes && clinicalNotes.length > 0"
      style="width:80%;margin-top:40px;margin-bottom:20px"
    >
      ...
      <div
        ...
        class="clinical-note"
        v-for="clinicalNote in clinicalNotes"
        :key="note"
      >
        <div class="note">
          {{ clinicalNote.note }}
    ...
    <div class="sub-heading" style="margin-top:40px;margin-bottom:0px">
      Reviewed Variants
    </div>
    <div v-if="!variantsInterpreted" class="case-summary">
      Variants are not currently reviewed. Please review them in the
      <a @click="gotoStep(2)"><strong>Review Variants</strong></a> step of the
      workflow
    </div>

    <div
      class="findings-section"
      v-for="interpretation in variantsByInterpretation"
      :key="interpretation.key"
    >
      <div
        v-if="
          interpretation.organizedVariants &&
            interpretation.organizedVariants.length > 0
        "
        class="interpretation-list"
      ></div>

      <template v-for="geneList in interpretation.organizedVariants">
        <template v-for="geneObject in geneList.genes">
  ...
  methods: {
    ...
    initClinicalNotes: function() {
      ...
      if (
        ...
        self.analysis.payload.phenotypes &&
        self.analysis.payload.phenotypes.length > 3
      ) {
        ...
        noteObjects.forEach(function(noteObject) {
          ...
          if (
            noteObject.phenolyzer_terms &&
            noteObject.phenolyzer_terms.length > 0
          ) {
