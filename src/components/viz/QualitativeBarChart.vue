...
export default {
  ...
  props: {
    ...
    colorScale: {
      ...
      default: d3
        .scaleOrdinal()
        .range(["#0080A8", "#2EB0CE", "#37D4EF", "#97E9F2"])
    }
  },
  data() {
    return {
  ...
  computed: {
    ...
    innerHeight() {
      return (
        this.height - this.margin.top - this.margin.bottom - this.topOffset
      );
    },
    innerWidth() {
  ...
  methods: {
    ...
    drawChart() {
      ...
      this.gMain
        ...
        .attr(
          "transform",
          `translate(0,${this.innerHeight - this.margin.bottom + 5})`
        )
        .call(xAxis);

      const bars = this.gMain.selectAll("rect").data(this.dataArray);
      bars.exit().remove();
      bars
        .enter()
        .append("rect")
      ...
      let labels = this.gMain.selectAll(".textLables").data(this.dataArray);
