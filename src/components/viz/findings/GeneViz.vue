...
import geneD3 from "../../../d3/findings/Gene.d3.js";
  ...
  methods: {
    ...
    update: function() {
      ...
      if (
        ...
        self.data.length > 0 &&
        self.data[0] != null &&
        Object.keys(self.data[0]).length > 0
      ) {
        this.geneChart.regionStart(this.regionStart);
        this.geneChart.regionEnd(this.regionEnd);
        this.geneChart.width(
          self.fixedWidth > 0 ? self.fixedWidth : this.$el.clientWidth
        );
