...
class GlobalApp {
  constructor() {
    ...
    // config files
    this.siteConfigUrl = {
      prod: "https://s3.amazonaws.com/gene.iobio.config/site-config.json",
      dev: "https://s3.amazonaws.com/gene.iobio.config/site-config-dev.json"
    };
    this.clinvarGenesUrl =
      "https://s3.amazonaws.com/gene.iobio.config/clinvar-counts.txt";
  ...
  initServices() {
    this.IOBIO_SERVICES = this.isOffline
      ? this.serverInstance
      : this.IOBIO_SOURCE;

    // End with "/" for IOBIO services
    if (
      this.IOBIO_SERVICES.indexOf("/", this.IOBIO_SERVICES.length - 1) == -1
    ) {
      this.IOBIO_SERVICES += "/";
    }
    ...
    this.HTTP_SERVICES =
      ...
      (this.isOffline ? this.serverInstance : this.HTTP_SOURCE);
    this.emailServer =
      (this.useSSL ? "wss://" : "ws://") + this.IOBIO_SOURCE + "email/";
  ...
  getClinvarUrl(build, launchedFromUtahMosaic) {
    if (this.IOBIO_SERVICES.indexOf("mosaic.chpc.utah.edu") == 0) {
      var clinvarUrls = {
        GRCh37:
          "https://mosaic.chpc.utah.edu/static/GRCh37/2019/clinvar_20191021.vcf.gz",
        GRCh38:
          "https://mosaic.chpc.utah.edu/static/GRCh38/2019/clinvar_20191021.vcf.gz"
      };
      return clinvarUrls[build];
    } else {
      var clinvarUrls = {
        GRCh37:
          "ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh37/archive_2.0/2018/clinvar_20181202.vcf.gz",
        GRCh38:
          "ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar/vcf_GRCh38/archive_2.0/2018/clinvar_20181202.vcf.gz"
      };
      return clinvarUrls[build];
    }
  }
  ...
  getGnomADUrl(build, chrom) {
    var gnomADSource = {
      genomes: {
        GRCh37:
          "https://storage.googleapis.com/gnomad-public/release/2.1.1/vcf/genomes/gnomad.genomes.r2.1.1.sites.CHROM-ALIAS.vcf.bgz",
        GRCh38:
          "https://storage.googleapis.com/gnomad-public/release/3.0/vcf/genomes/gnomad.genomes.r3.0.sites.chrCHROM-ALIAS.vcf.bgz"
      },
      exomes: {
        GRCh37:
          "http://storage.googleapis.com/gnomad-public/release/2.1.1/vcf/exomes/gnomad.exomes.r2.1.1.sites.CHROM-ALIAS.vcf.bgz",
        GRCh38:
          "ftp://ftp.ensembl.org/pub/data_files/homo_sapiens/GRCh38/variation_genotype/gnomad/r2.1/exomes/gnomad.exomes.r2.1.sites.grch38.chrCHROM-ALIAS_noVEP.vcf.gz"
      }
    };
